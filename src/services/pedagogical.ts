    ...
    async getAttentionQueue() {
        const studentsInAttention = await prisma.student.findMany({
            include: {
                ...
                occurrences: {
                    select: { id: true, type: true, gravity: true, description: true, createdAt: true },
                }
            }    
        });

        const attentionQueue = studentsInAttention.map(student => {
            const totalOccurrences = student.occurrences.length;
            
            const hasSevereAlert = student.occurrences.some(occurrence => occurrence.gravity.toLowerCase() === "grave");
        ...
        }).filter(student => student.status !== "Normal" || student.hasSevereAlert);
